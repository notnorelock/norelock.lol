...
/**
 * Keeps the fields a human edited, so a re-run does not wipe them. Reads the
 * generated API file, whose track list is plain JSON.
 */
function readExisting() {
  ...
  // Start after the '=' so the '[]' in the `ApiTrack[]` annotation is skipped.
...
const apiFile = [
  ...
  '//',
  '// The API routes cannot import from src/: Vercel compiles api/ in place and',
  '// does not bundle modules from outside it. This is their own copy, and it',
  '// is also what /api/tracks serves to the browser.',
  '',
  ...
  '  year: number;',
  '  duration: number;',
  '  /** Path inside media/. Server-side only, stripped before sending. */',
  '  src: string;',
  '  cover: string;',
  '  album?: string;',
  '  albumTitle?: string;',
